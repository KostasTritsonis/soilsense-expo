...
export async function fetchWeatherData(
  ...
  // In a real app, you would store API keys securely
  ...
  try {
    ...
    forecastData.list.forEach((entry) => {
      ...
      if (!dailyForecast[dateKey]) {
        ...
        dailyForecast[dateKey].high =
          `${Math.max(parseInt(prev.high), Math.round(entry.main.temp_max))}°C`;
        dailyForecast[dateKey].low =
          `${Math.min(parseInt(prev.low), Math.round(entry.main.temp_min))}°C`;
      }
    });
